...
const Sphere: React.FC<SphereProps> = ({ parent }) => {
  ...
  useEffect(() => {
    ...
    // Create camera
    const camera = new THREE.PerspectiveCamera(35, 1.2, 1, 1000);
    ...
    const items: number[] = [-1, -0.5, 0.5, 1];
    controls.autoRotateSpeed = items[Math.floor(Math.random() * items.length)];
